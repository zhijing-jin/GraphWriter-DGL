...
class RDFFileReader:
    def __init__(self, file_name, verbose=False):
        ...
        if verbose and self.cnt: show_var(["self.cnt"])
    ...
    def extract_sentences(self, lex):
        ...
        for s in sentences:
            ...
            new_s_t, tex, tem, uniq_tag2ent = \
                self.fix_sentence(s_tripleset, text, template, tag2ent)
    ...
    def fix_document(self, s_tripleset_raw, template, text, tag2ent):
        ...
        # flatten tripleset
        s_tripleset = flatten_list(s_tripleset)
...
class WebNLGDataReader(DataReader):
    def __init__(self, set: DataSetType):
        ...
        readers = [RDFFileReader(f) for f in files]
        data = flatten_list(r.data for r in readers)
        cnt = sum(r.cnt for r in readers)
        show_var(['cnt'])
...
#
#
# rels = [triple[1] for line in data for triple in line['triples']]
